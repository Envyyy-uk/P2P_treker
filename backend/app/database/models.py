...
class Quote1s(Base):
    ...
    З міграції 9bd1a8fa90b4 (Фаза 2.1) фактична таблиця в БД —
    RANGE-партиційована по днях за `timestamp`, з композитним PRIMARY KEY
    (id, timestamp) — так вимагає Postgres для партиційованих таблиць.
    Ця модель навмисно цього не відображає (тут лише id — PK), бо
    використовується виключно для побудови INSERT-запитів
    (`app/repositories/market_data.py`), не для DDL чи автогенерації
    Alembic-міграцій (усі міграції в проєкті пишуться вручну).
    """
